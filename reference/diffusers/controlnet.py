...
from weight_files import (
    LocalWeightFile, model_content_sha256, checked_safetensors_path, file_sha256, verify_weight_file,
)
...
def _identities(paths: list[Path]) -> list[LocalWeightFile]:
    ...
    for path in sorted(set(paths)):
        ...
        result.append(LocalWeightFile(str(path.absolute()), str(path.resolve()), model_content_sha256(path), path.stat().st_size))
    return result
